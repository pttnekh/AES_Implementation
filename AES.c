...
int main() {
    ...
    // 4. 결과 출력: 라운드별로 4워드(16바이트)씩 끊어서 출력
    printf("--- AES-128 Key Expansion 결과 ---\n");
    
    for (int round = 0; round <= 10; round++) {
        printf("Round %2d Key: ", round);
        
        // 각 라운드마다 4개의 워드(w)가 사용됨
        for (int i = 0; i < 4; i++) {
            printf("%08X ", w[round * 4 + i]);
        }
        printf("\n");
    }
